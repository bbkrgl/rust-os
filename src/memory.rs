...
unsafe fn active_level4_table(physical_mem_offset: VirtAddr) -> &'static mut PageTable {
    ...
    get_table_from_frame(level4_table_frame, physical_mem_offset)
}

unsafe fn get_table_from_frame(
    frame: PhysFrame,
...
fn translate_addr_inner(addr: VirtAddr, physical_mem_offset: VirtAddr) -> Option<PhysAddr> {
    ...
    let l4table = unsafe { active_level4_table(physical_mem_offset) };
    let table_indexes = [
        addr.p4_index(),
        addr.p3_index(),
        addr.p2_index(),
    ...
    let mut table = l4table;
    let mut frame = PhysFrame::start_address(self);
    for index in table_indexes {
        let entry = &table[index];
        table = match entry.frame() {
            Ok(fr) => unsafe {
                frame = fr;
                get_table_from_frame(fr, physical_mem_offset)
            },
    ...
    Some(PhysAddr::new(
        frame.start_address().as_u64() + addr.as_u64(),
    ))
}
*/

pub unsafe fn init(physical_mem_offset: VirtAddr) -> OffsetPageTable<'static> {
    let l4table = active_level4_table(physical_mem_offset);
    OffsetPageTable::new(l4table, physical_mem_offset)
}
...
pub fn create_example_mapping(
    ...
    map_to_result.expect("failed").flush();
}
